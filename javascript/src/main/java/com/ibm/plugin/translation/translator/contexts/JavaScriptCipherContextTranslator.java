...
import com.ibm.mapper.mapper.jca.JcaCipherMapper;
...
public final class JavaScriptCipherContextTranslator implements IContextTranslation<Tree> {
    ...
    public @Nonnull Optional<INode> translate(
        ...
        if (value instanceof ValueAction<Tree> || value instanceof Algorithm<Tree>) {
            if (detectionContext instanceof DetectionContext context
                    && context.get("algorithm").map(v -> v.equalsIgnoreCase("RSA")).orElse(false)) {
                return Optional.of(new RSA(PublicKeyEncryption.class, detectionLocation));
            }
            if ("RSA".equalsIgnoreCase(value.asString().trim())) {
                return Optional.of(new RSA(PublicKeyEncryption.class, detectionLocation));
            }
            ...
            Optional<INode> jcaResult =
                    jcaCipherMapper
                            .parse(normalizeNodeCipherName(valueStr), detectionLocation)
                            .map(node -> node);
            if (jcaResult.isPresent()) {
                return jcaResult;
            }
    ...
    /**
     * Converts Node.js/OpenSSL cipher names (e.g. {@code aes-256-cbc}) to JCA format ({@code
     * AES/CBC}).
     */
    @Nullable private static String normalizeNodeCipherName(@Nonnull String nodeCipher) {
        String lower = nodeCipher.toLowerCase();
        if (lower.contains("/")) {
            return nodeCipher;
        }
        String[] parts = lower.split("-");
        if (parts.length >= 3 && "aes".equals(parts[0])) {
            return "AES/" + parts[parts.length - 1].toUpperCase();
        }
        if (lower.startsWith("des-ede3-") || lower.startsWith("des-ede-")) {
            int lastDash = lower.lastIndexOf('-');
            return "DESEDE/" + lower.substring(lastDash + 1).toUpperCase();
        }
        if (parts.length >= 2 && "chacha20".equals(parts[0])) {
            return "CHACHA20/" + parts[parts.length - 1].toUpperCase();
        }
        return nodeCipher;
    }
}
