...
public final class JavaScriptDetectionEngine implements IDetectionEngine<Tree, JavaScriptSymbol> {
    ...
    public void resolveValuesInOuterScope(
        ...
        resolveValuesInInnerScope(
                ...
                .map(
                        resolvedValue ->
                                new ValueDetection<>(
                                        resolvedValue, detectableParameter, expression, expression))
    ...
    private boolean matches(@Nonnull CallExpressionTree call) {
        logExpectedRule();
        DebugLogger.log(
                "Matching call "
                        + call.objectType()
        ...
        DebugLogger.log("Match result: " + matched);
        return matched;
    }

    private boolean matches(@Nonnull CallExpressionWithBlockTree call) {
        logExpectedRule();
        DebugLogger.log(
                "Matching call "
                        + call.objectType()
    ...
    @SuppressWarnings("unchecked")
    private void logExpectedRule() {
        if (detectionStore.getDetectionRule().is(DetectionRule.class)) {
            DetectionRule<Tree> rule = (DetectionRule<Tree>) detectionStore.getDetectionRule();
            DebugLogger.log(
                    "Against rule "
                            + rule.matchers().getInvokedObjectTypeStringsSerializable()
                            + "."
                            + rule.matchers().getMethodNamesSerializable()
                            + rule.matchers().getParameterTypesSerializable());
        }
    }
