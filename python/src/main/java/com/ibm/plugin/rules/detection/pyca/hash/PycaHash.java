 ...
 */
package com.ibm.plugin.rules.detection.pyca.hash;
...
public final class PycaHash {
    ...
    @Nonnull
    private static final Supplier<List<IDetectionRule<Tree>>> WRAPPER_RULES =
            Memoize.of(PycaHash::wrapperRule);

    @Nonnull
    public static List<IDetectionRule<Tree>> wrapperRule() {
        return List.of(HASH_WRAPPER);
    }

    @Nonnull
    public static List<IDetectionRule<Tree>> wrapperRules() {
        return WRAPPER_RULES.get();
    }
}
