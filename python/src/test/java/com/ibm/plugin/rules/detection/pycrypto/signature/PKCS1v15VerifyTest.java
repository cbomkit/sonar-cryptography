...
public class PKCS1v15VerifyTest extends TestBase {

    @Test
    void test() {
        PythonCheckVerifier.verify(
                "src/test/files/rules/detection/pycrypto/signature/PKCS1v15VerifyTestFile.py",
                this);
    }
    ...
    public void asserts(
        ...
        if (findingId == 2) {
            ...
            assertThat(sig.asString()).isEqualTo("RSA-PKCS1-1.5-SHA-256");

            INode oid = sig.getChildren().get(Oid.class);
            assertThat(oid).isNotNull();
            assertThat(oid.getChildren()).isEmpty();
            assertThat(oid.asString()).isEqualTo("1.2.840.113549.1.1.11");

            INode padding = sig.getChildren().get(Padding.class);
