...
public class PSSVerifyTest extends TestBase {

    @Test
    void test() {
        PythonCheckVerifier.verify(
                "src/test/files/rules/detection/pycrypto/signature/PSSVerifyTestFile.py", this);
    }
    ...
    public void asserts(
        ...
        if (findingId == 2) {
            ...
            assertThat(sigValue.asString()).isEqualTo("VERIFY");

            // translation
            assertThat(nodes).hasSize(1);
            INode pss = nodes.get(0);
            assertThat(pss.getKind()).isEqualTo(ProbabilisticSignatureScheme.class);
            assertThat(pss.getChildren()).hasSize(4);
            assertThat(pss.asString()).isEqualTo("RSA-PSS");

            INode oid = pss.getChildren().get(Oid.class);
            assertThat(oid).isNotNull();
            assertThat(oid.getChildren()).isEmpty();
            assertThat(oid.asString()).isEqualTo("1.2.840.113549.1.1.10");

            INode key = pss.getChildren().get(Key.class);
            ...
            assertThat(pkeOid.getChildren()).isEmpty();
            assertThat(pkeOid.asString()).isEqualTo("1.2.840.113549.1.1.1");

            INode md = pss.getChildren().get(MessageDigest.class);
            ...
            INode verify = pss.getChildren().get(Verify.class);
            assertThat(verify).isNotNull();
            assertThat(verify.getChildren()).isEmpty();
