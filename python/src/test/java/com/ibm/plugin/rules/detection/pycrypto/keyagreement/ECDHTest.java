...
public class ECDHTest extends TestBase {

    @Test
    void test() {
        PythonCheckVerifier.verify(
                "src/test/files/rules/detection/pycrypto/keyagreement/ECDHTestFile.py", this);
    }
    ...
    public void asserts(
        ...
        if (findingId == 1) {
            ...
            assertThat(root).isInstanceOf(ECDH.class);
            assertThat(root.asString()).isEqualTo("ECDH");

            assertThat(root.getChildren().get(Oid.class)).isNotNull();
            assertThat(root.getChildren().get(Oid.class).asString()).isEqualTo("1.3.132.1.12");
