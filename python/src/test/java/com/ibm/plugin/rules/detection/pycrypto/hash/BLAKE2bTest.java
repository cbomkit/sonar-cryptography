...
public class BLAKE2bTest extends TestBase {

    @Test
    void test() {
        PythonCheckVerifier.verify(
                "src/test/files/rules/detection/pycrypto/hash/BLAKE2bTestFile.py", this);
    }
    ...
    public void asserts(
        ...
        assertThat(value.asString()).isEqualTo("BLAKE2b");

        assertThat(nodes).hasSize(1);
        INode md = nodes.get(0);
        assertThat(md).isInstanceOf(MessageDigest.class);
        assertThat(md.getChildren()).hasSize(2);
        assertThat(md.asString()).isEqualTo("BLAKE2b");

        INode saltLength = md.getChildren().get(SaltLength.class);
        assertThat(saltLength).isNotNull();
        assertThat(saltLength.getChildren()).isEmpty();
        assertThat(saltLength.asString()).isEqualTo("128");
